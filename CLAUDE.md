...
- `bun run desktop:e2e` (после `desktop:build`) — `packages/desktop/e2e` через Playwright `_electron.launch` на собранном `SimCity.app`: окно без фокуса, `__sim` отвечает, тестовый город держит 60 fps. `SIMCITY_TEST_WINDOW=1` не активирует приложение, делает окно нефокусируемым, поверх остальных и прозрачным для кликов; окно закрывается после каждого теста.
