...
описано ниже. Почему — `docs/live-debug.md`. Исключение одно и законное: когда проверяется сам
клавиатурный ввод — хоткеи, фокус поля, — `send_keys` и есть предмет теста; только не на паузе.
...
**Пауза замораживает и синтетические клавиши.** `brp_extras/send_keys` отпускает клавишу по
таймеру на виртуальном времени, а `{"speed":"paused"}` это время останавливает. На паузе нажатая
клавиша так и остаётся зажатой, повторное нажатие не даёт `just_pressed`, и хоткей выглядит
сломанным, хотя сломан тест. Для проверки ввода ставьте `{"speed":"x1"}`, а не паузу.
...
2. `simcity/sim {"speed":"paused","hour":12}` — заморозить сцену, чтобы кадры отличались только
   тем, что вы меняете. Это для правок через `simcity/command` и для кадров; если проверяете
   клавиатурный ввод, не ставьте паузу — см. раздел 5.
...
| Хоткей «не срабатывает» повторно через `send_keys` | Симуляция на паузе: отпускание клавиши ждёт виртуального времени, клавиша висит зажатой | Для проверки ввода `{"speed":"x1"}`, не `paused` |
