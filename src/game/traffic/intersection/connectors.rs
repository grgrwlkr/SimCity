...
pub(crate) fn rewrite_intersection_connectors(
    ...
    for (e, mut v, debug_opt) in q_vehicles.iter_mut() {
        ...
        if let Some(route) = path_pool.remaining_from(v.path_handle, v.path_cursor) {
            ...
            if route.len() >= 3
                && !is_intersection_tile(&grid, route[0])
                && let Some((i, j)) = first_intersection_segment(route, &grid)
            {
                        ...
                        if let Some((connector, anchor)) = build_connector_path(
                            ...
                            if existing_segment != connector.as_slice() {
                                ...
                                let mut new_route = Vec::with_capacity(
                                    route.len() - existing_segment.len() + connector.len(),
                                );
                                new_route.extend_from_slice(&route[..i]);
                                new_route.extend(connector);
                                new_route.extend_from_slice(&route[j..]);
...
fn build_connector_path(
    ...
    let mut path_a = axis_first_path(entry_tile, anchor, Some(entry_dir), cluster)
        .or_else(|| bfs_path(entry_tile, anchor, cluster))?;
    let path_b = axis_first_path(anchor, exit_tile, Some(exit_dir), cluster)
        .or_else(|| bfs_path(anchor, exit_tile, cluster))?;
...
/// Build a deterministic axis-first Manhattan path inside a cluster.
fn axis_first_path(
    start: TilePos,
    end: TilePos,
    ...
    if let Some(path) = axis_path_with_order(start, end, primary, cluster) {
        return Some(path);
    }
    axis_path_with_order(start, end, secondary, cluster)
}
...
fn axis_path_with_order(
    ...
    walk_axis(&mut cur, end, first_axis, cluster, &mut path)?;
    walk_axis(&mut cur, end, first_axis.other(), cluster, &mut path)?;
    ...
    match axis {
        PrimaryAxis::X => {
            ...
            for _ in 0..(end.x - cur.x).abs() {
                cur.x += step;
                if !cluster.tiles_set.contains(cur) {
                    return None;
                }
                path.push(*cur);
            }
...
/// Fallback BFS inside a cluster (guarantees a path if the cluster is connected).
fn bfs_path(start: TilePos, end: TilePos, cluster: &ClusterCache) -> Option<Vec<TilePos>> {
    if start == end {
        return Some(vec![start]);
    }
