...
**Уточнение по источнику “пробок” (MVP решение):**
- В текущей архитектуре `TrafficOccupancy` обновляется в `FixedUpdate` в `GameSet::PostSim`, а планирование маршрутов (A*) выполняется при спавне/перепланировании машин в `GameSet::Sim`.
- Поэтому на первом этапе используем **вариант A**: A* учитывает `TrafficOccupancy` как снимок **предыдущего fixed-tick** (лаг 1 tick). Это упрощает реализацию и не требует перестройки порядка систем. При необходимости позже можно перейти к варианту без лага (перестановка/разделение систем или отдельный “current tick occupancy” ресурс).
...
**Уточнение по хоткеям/подсказкам (важно для UX):**
- При любых изменениях инструментов/режимов нужно держать в синхронизации:
  - фактические хоткеи (`1..5`, и т.п.),
  - подписи в top bar,
  - текст подсказок/логов “Controls”.
- В текущем MVP возможны рассинхроны (например, упоминание инструмента, которого нет). При реализации этой фазы — привести к консистентному набору.
...
**Уточнение (обязательное для “строго только вдоль дорог”):**
- Нельзя полагаться только на UI/курсор: нужно **валидировать и при применении команды** `GameCommand::SetZone` (в `apply_game_commands_to_grid`), иначе зону можно выставить “обходным путём”.
- В текущем MVP коде уже есть проверки на `water`, но **нужно добавить** запрет зонирования на:
  - road tiles (`cell.road != None/true`),
  - tiles со зданием (`cell.building.is_some()`),
  - tiles без соседней дороги (`has_adjacent_road == false`).
- Подсветка: при активном инструменте зонирования показываем overlay “можно/нельзя” (как описано выше), чтобы игрок видел допустимые тайлы заранее.
...
**Уточнение по версии сейвов на ближайший этап:**
- Пока **совместимость сейвов не приоритет**: можно продолжать использовать **SaveGame v1** и **модифицировать v1** по мере необходимости (без миграции/поддержки старых слотов). Когда дизайн стабилизируется — перейти к v2 с явной миграцией.
...
## 10. Выполненные улучшения (2024-2025)

### ✅ UI и UX улучшения

| Задача                                                           | Статус      | Дата    |
| ---------------------------------------------------------------- | ----------- | ------- |
| **UI Redesign** — разделение на status bar, toolbar, sidebar     | ✅ Выполнено | 2025-01 |
| **Tooltips** — подсказки для всех UI элементов                   | ✅ Выполнено | 2025-01 |
| **Shortcuts Panel** — панель клавиатурных сокращений (клавиша ?) | ✅ Выполнено | 2025-01 |
| **Improved Graphs** — улучшенные графики с hover                 | ✅ Выполнено | 2025-01 |
| **UI Settings** — панель настроек UI (F10)                       | ✅ Выполнено | 2025-01 |
| **Notifications** — система уведомлений о событиях               | ✅ Выполнено | 2025-01 |

### ✅ Система дорог и трафика

| Задача                                                            | Статус      | Дата    |
| ----------------------------------------------------------------- | ----------- | ------- |
| **One-Way Roads** — поддержка односторонних дорог (RoadFlow enum) | ✅ Выполнено | 2025-01 |
| **Turn Lanes** — полосы поворота (LaneType enum)                  | ✅ Выполнено | 2025-01 |
| **Traffic Light Stopping** — остановка машин на красный свет      | ✅ Выполнено | 2025-01 |
| **Traffic Light Pathfinding** — учёт светофоров в pathfinding     | ✅ Выполнено | 2025-01 |
| **Yellow Light Phase** — жёлтый сигнал светофора                  | ✅ Выполнено | 2025-01 |
| **Intersection Priority** — правила приоритета (yield/stop)       | ✅ Выполнено | 2025-01 |

### ✅ Система зданий и экономики

| Задача                                                    | Статус      | Дата    |
| --------------------------------------------------------- | ----------- | ------- |
| **Building Levels** — система уровней зданий с апгрейдами | ✅ Выполнено | 2025-01 |
| **Land Value System** — система стоимости земли           | ✅ Выполнено | 2025-01 |
| **Pollution System** — система загрязнения                | ✅ Выполнено | 2025-01 |

### ✅ Система отмены действий

| Задача                                                   | Статус      | Дата    |
| -------------------------------------------------------- | ----------- | ------- |
| **Undo/Redo** — система отмены действий (Ctrl+Z, Ctrl+Y) | ✅ Выполнено | 2025-01 |

---

## 11. План дальнейших улучшений

### Приоритет 1: Критические улучшения геймплея

#### 1.1 Weighted Pathfinding (Congestion влияет на маршруты)
**Статус:** 🔲 Не реализовано  
**Описание:** A* должен учитывать пробки при выборе маршрута. Сейчас используется uniform cost = 1 для всех рёбер.

**Требования:**
- Передать `TrafficOccupancy` в `find_road_path_cached`
- Вычислять `step_cost` на основе congestion текущего тайла
- Формула: `w = base_cost × (1 + k × congestion)`

**Файлы для изменения:**
- `src/game/transport.rs` — функция `find_road_path_cached`

#### 1.2 RCI Demand System
**Статус:** 🔲 Частично реализовано (базовая структура есть)  
**Описание:** Система спроса на жильё, коммерцию и промышленность должна влиять на рост зданий.

**Требования:**
- Residential demand ↑ когда jobs > citizens
- Commercial demand ↑ когда `UnmetShoppingDemand` высокий
- Industrial demand ↑ когда employment rate низкий
- Интеграция с системой роста зданий

**Файлы для изменения:**
- `src/game/demand.rs` — логика расчёта спроса
- `src/game/buildings.rs` — использование спроса при росте

#### 1.3 Building Demolition / Decay
**Статус:** 🔲 Не реализовано  
**Описание:** Здания должны "умирать" при отсутствии дороги или негативном спросе.

**Требования:**
- Timer на здание без road access → despawn
- Decay при happiness < threshold
- Визуальная индикация разрушения

**Файлы для изменения:**
- `src/game/buildings.rs` — система decay

### Приоритет 2: Визуализация и UX

#### 2.1 Полноценные Overlays
**Статус:** 🔲 Частично реализовано  
**Описание:** Height, Water, Roads overlays должны иметь специальную визуализацию.

**Требования:**
- Height → градиент серого по `cell.height`
- Water → голубой с прозрачностью
- Roads → контрастный цвет дорог
- Service Coverage → радиусы покрытия служб

**Файлы для изменения:**
- `src/game/map/mod.rs` — система overlay rendering

#### 2.2 Mini-map улучшения
**Статус:** ✅ Базовая реализация есть  
**Описание:** Улучшить minimap с интерактивностью и дополнительной информацией.

**Требования:**
- Клик на minimap → перемещение камеры
- Отображение активных событий
- Отображение покрытия служб

**Файлы для изменения:**
- `src/game/ui.rs` — функция `render_minimap_in_sidebar`

#### 2.3 Building Info Popup улучшения
**Статус:** ✅ Базовая реализация есть  
**Описание:** Расширить информацию о здании.
...
**Файлы для изменения:**
- `src/game/ui.rs` — функция `building_popup_ui`

### Приоритет 3: Технические улучшения

#### 3.1 Persistence v2
**Статус:** 🔲 Не реализовано  
**Описание:** Расширить систему сохранения для новых фич.

**Требования:**
- Сохранение RoadKind, RoadFlow, LaneType
- Сохранение LandValueIndex, PollutionIndex
- Сохранение Building levels
- Сохранение UI Settings
- Миграция из v1 в v2

**Файлы для изменения:**
- `src/game/persistence.rs`
- `src/game/persistence_contract.rs`

#### 3.2 Building Sync на Load
**Статус:** 🔲 Частично реализовано  
**Описание:** Убедиться, что все building entities синхронизированы с grid при загрузке.

**Требования:**
- Система `sync_building_entities_from_grid` в `RenderSync` set
- При загрузке пересоздавать building entities из `MapGrid.building`
- Учитывать уровни зданий

**Файлы для изменения:**
- `src/game/map/mod.rs` — система синхронизации

#### 3.3 Config Externalization
**Статус:** 🔲 Не реализовано  
**Описание:** Вынести константы в `.ron` конфиги.

**Требования:**
- `economy_config.ron` — экономические параметры
- `traffic_config.ron` — параметры трафика
- `building_config.ron` — параметры зданий
- `ui_config.ron` — настройки UI

**Файлы для изменения:**
- `src/game/config_loader.rs` — расширение загрузки конфигов

### Приоритет 4: Производительность

#### 4.1 Hierarchical Pathfinding
**Статус:** 🔲 Не реализовано  
**Описание:** Для больших карт (256×256+) A* становится дорогим.
...
**Файлы для изменения:**
- `src/game/transport.rs` — новая система pathfinding

#### 4.2 Chunk-based Rendering
**Статус:** 🔲 Не реализовано  
**Описание:** Сейчас все тайлы — отдельные спрайты (16K entities для 128×128).
...
**Файлы для изменения:**
- `src/game/map/mod.rs` — система рендеринга

#### 4.3 Vehicle LOD
**Статус:** 🔲 Не реализовано  
**Описание:** При большом количестве машин нужна оптимизация.
...
**Файлы для изменения:**
- `src/game/traffic.rs` — система LOD

### Приоритет 5: Расширение контента

#### 5.1 Public Transport
**Статус:** 🔲 Частично реализовано (базовая структура есть)  
**Описание:** Система общественного транспорта.

**Требования:**
- Bus routes/stops
- Metro lines
- Trip abstraction через `TripPurpose`
- Визуализация маршрутов

**Файлы для изменения:**
- `src/game/public_transport.rs` — расширение функциональности

#### 5.2 Дополнительные типы событий
**Статус:** 🔲 Не реализовано  
**Описание:** Расширить систему событий.
...
**Файлы для изменения:**
- `src/game/emergencies.rs` — новые типы событий

#### 5.3 Апгрейд зданий (расширение)
**Статус:** ✅ Базовая реализация есть  
**Описание:** Расширить систему апгрейдов зданий.

**Требования:**
- Больше уровней (сейчас до 3)
- Специальные улучшения
- Визуальные эффекты

**Файлы для изменения:**
- `src/game/buildings.rs` — система апгрейдов

---

*Документ обновлён: Январь 2025*
*На основе анализа кодовой базы и master-plan.md*
*Последние изменения: Добавлен раздел о выполненных улучшениях и план дальнейших улучшений*
