...
| 1c | построение лейнлетов, матрицы конфликтов, `find_route` с sidecar (кластеры ушли в 1b) | 45 | ворота этапа из программы: `find_route` на 200 парах совпадает с Rust вместе с sidecar |
...
**1c (2026-09-11).** 45 тестов зелёные, ворота пройдены, а с ними и ворота этапа 1 из программы. На тестовом городе 596 лейнлетов, матрицы 18 перекрёстков и `find_route` с sidecar на 200 парах полос совпадают с Rust: 112 пар дают маршрут, 110 из них идут через перекрёстки, у 150 пар ненулевой сид джиттера. Тест не пустой: обнулённый джиттер роняет маршруты. Сдвиг штрафа полосы или поворота на единицу маршрутов не меняет, его ловит сверка конфига. Отклонения описаны в «1c: файлы и контракты». Вдобавок примеры получили общий `examples/common/mod.rs`; `dump_map` и `dump_routes` переведены на него, их фикстуры перегенерированы и совпали байт-в-байт. Замер: `findRoute` 154 мс на 200 маршрутов (~0.8 мс на маршрут, джиттер на `BigInt` — кандидат на оптимизацию к этапу 2), полный Vitest 157 тестов за 0.91 с, e2e 6.2 с. Rust после поднятия видимости: `cargo fmt --check` чистый, `cargo clippy --all-targets --all-features -D warnings` чистый при `-A clippy::derivable_impls` (это предупреждение уже есть на main в `simcity_sim`), 79 тестов `cargo test -p simcity_sim lanelet` зелёные.
