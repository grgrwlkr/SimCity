...
> **Поправка к программе (2026-09-11, после 2b).** Rust больше не эталон. Всё, что можно улучшить при переносе, улучшаем обязательно. Фикстуры Rust остаются регрессионными воротами, пока его поведение правильное. Где порт становится лучше, фикстура перегенерируется или снимается, а отклонение и его причина записываются сюда. Для этапа 2 это значит:
>
> - степень в IDM — детерминированное f64-произведение с одним округлением (libm macOS не повторяем);
> - порядок обхода машин — порядок слотов с тай-брейком по `seq`, архетипы Bevy не эмулируются;
> - затор на кресте из ворот 2b разбирается и чинится в порте;
> - ворота 2c вместо «0 расхождений с Rust» проверяют инварианты и отсутствие заторов.
