...
- **Портировано 39 тестов 3b, плюс 1 из `services/coverage.rs` и 1 из `map/tests.rs`.** Добавлены тесты поведения, которых в Rust не было: назначение работы по классу и дороге, потеря работы со зданием, заселение, поездка на работу и домой, машины ушедших жителей, вход здания со стороны дороги, класс по футпринту, живой город.
- **Ворота — `gate3b.test.ts`.** Город из команд живёт 30 дней без сценария. Все три месячных отчёта сходятся с казной, у жителей есть работа, машины на дороге везут жителей.
- **Меню:** «Город» остался демонстрацией трафика на 2000 пассажиров, но без зон. «Живой город» — тот же генератор со станциями, растёт сам.
- **Прогон:** vitest 76 файлов / 454 теста, typecheck, lint, e2e 40 из 40 — зелёные.

**Отклонения от Rust**

1. Станция службы выводится из записей: работающее здание службы у дороги.
2. Стоимость дорог и зданий — строка `Construction` журнала. Ставки, финансирование и кредиты сбрасываются с новой игрой, в Rust переходили в следующую.
3. Загрязняют только работающие заводы. `GenerateMap` сбрасывает загрязнение, стоимость земли и поля. Земля сбрасывается к середине, в Rust — к нулю, то есть к низкому классу.
4. Четыре теста спроса в Rust проверяли арифметику в теле теста; формулы стали функциями.
5. Дом и работа жителя — id здания. Нанимают только работающие здания. Таймеры тянутся на каждого жителя, а не на здание. Прибытия читаются в том же тике.
6. **Поездка выезжает и паркуется на тайле футпринта у дороги, а не на якоре.** Замер на городе ворот до правки: спавн выбросил 213 поездок из 370. У домов, чей якорь смотрит от дороги, жители висели «в пути» до таймаута. После правки — 0.
7. **Класс здания — по средней стоимости земли на футпринте, а не под якорем.** До правки все места работы были High: земля у дороги 0,7. Работали 15 жителей из 195. После правки работают 90 из 90.
8. Ворота трафика грузят тестовый город без зон: с настоящим спросом там растут жители, и их id совпадают с индексами пассажиров обвязки.

**Замеры и находка.** Город ворот на карте 128×128 — 0,33 мс на тик.

| день | население | работают | поездки / прибытия | машин с жителями, максимум | казна |
|---|---|---|---|---|---|
| 6 | 168 | 0 | 0 / 0 | 0 | 23 401 |
| 11 | 143 | 114 | 156 / 18 | 125 | 27 777 |
| 31 | 89 | 90 | 458 / 232 | 201 | 39 899 |

После 6-го дня население падает. Жители ездят без перерыва: работа 5–9 с и магазин 2–5 с — калибровка Rust. Единственная дорога на 104 полосы-тайла стоит почти полная. Жилой спрос следует за числом работ. Спрос на торговлю и промышленность уходит в минус из-за штрафа за пробки и полной занятости. Ворота всего этапа требуют роста населения первые 10 дней, поэтому это решается в 3c: распорядок дня жителя или калибровка.
