...
- `bun run desktop:build:test` — та же сборка, но страница собрана с `SIMCITY_SIM_API=1` (есть `window.__sim`), а в `out/build-info.json` стоит `test: true` (DevTools и меню по умолчанию остаются); результат — `packages/desktop/release/test/mac-arm64/SimCity.app`. Обе сборки пишут в `build-info.json` коммит и признак грязного дерева.
- `bun run desktop:e2e` — `packages/desktop/e2e/package.spec.ts` (иконка и фьюзы релизного `.app`) и `shell.spec.ts` (Playwright `_electron.launch` на клоне тестовой сборки, см. «Иконка и фьюзы» и «Сейвы файлами и `__sim` вне релиза»), всё с `SIMCITY_TEST_WINDOW=1`. Нужны обе сборки одного коммита: без тестовой или с чужим коммитом `shell.spec.ts` падает с именем команды, а не пропускается.
...
- **Сейвы.** `packages/desktop/src/saves.ts`: файлы `<userData>/saves/slot<n>.json`. Запись и удаление одного слота идут по очереди (цепочка промисов на слот): последний запрошенный сейв и есть итоговый, целиком. Каждая запись — во временный файл со своим именем (`slot<n>.json.<pid>-<n>.partial`), `fsync`, переименование; при ошибке временный файл удаляется. слот — целое 1–255 (как `u8` слотов Rust, без его `max(1)`: 0 отвергается). Обработчики `ipcMain` сначала проверяют отправителя (главный фрейм окна с разрешённым origin), затем номер — до любого обращения к диску; байты сейва — только `ArrayBuffer` или его представление. Preload добавляет в `window.simcityDesktop` ровно `save/load/list/remove` — **отклонение** от критерия E1 («только save и load»): `SaveStore` требует ещё `list` и `remove`; каждый передаёт в main только номер слота (и байты у `save`). Диалога нет. На стороне страницы `packages/app/src/saves/desktopSaveStore.ts`: сейв уходит из воркера запросом `save`, возвращается запросом `load`; `createSaveStore` выбирает файлы по наличию `window.simcityDesktop`, иначе OPFS воркера.
...
- **Релиз без DevTools (Q6 = a).** В релизе `webPreferences.devTools: false` и меню из `appMenu`, `editMenu`, `windowMenu` — без View, то есть без перезагрузки и DevTools. Решает `devToolsAllowed` в `main.ts`: DevTools есть только у распакованного (dev) и у тестовой сборки; сборка, чей `build-info.json` не читается, считается релизом.
- **Обфусцированная сборка.** Флаг выбрасывает модуль `window.__sim` до обфускации (define и удаление мёртвого кода идут раньше `renderChunk`), так что поиск по её asar проверяет то же самое.
- **Замер fps тестового города.** Планка та же (58 у страницы, 55 композитных). Прогрев 5 с, затем до трёх окон по 10 с, окно засчитывается, когда в нём держатся обе метрики, и таких окон должно быть большинство; два прошедших окна завершают замер. Разовый провал в окне больше не решает исход; при постоянной загрузке GPU тест падает, как и должен.
