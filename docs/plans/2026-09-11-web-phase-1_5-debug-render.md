...
- Найдено после закрытия ворот, при показе в скрытой панели браузера: рендерер, созданный на канвасе 0×0, сыпал ошибками валидации WebGPU. Теперь буфер рисования не меньше пикселя, а кадры ждут настоящего размера; пин — `drawingStartsOnceAZeroSizeCanvasGetsASize`. Сворачивать канвас в тесте приходится init-скриптом: ответ, подменённый через `page.route`, в WebKit теряет COOP/COEP.
