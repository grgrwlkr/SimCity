...
`city` (`fitMap`, орто), `district` (пожарная станция, 1 м/пиксель, орто), `street` (0,2, перспектива), `corner` (северо-восточный
угол станции, 0,1, перспектива: видны и окна, и тень). Камера ставится после того, как карта нарисована: поставленную раньше
первый кадр вписывал обратно. Каждый кадр снимается на двух страницах в двух контекстах браузера и сравнивается между ними
и с эталоном `e2e/fixtures/render/<ракурс>-<час>.png`; `E2E_RENDER_UPDATE=1` пишет эталоны. Эталоны — TS-кадры этой ветки
под SwiftShader, их утверждает пользователь в конце волны.
- Метрика: среднее по кадру наибольшей из разниц каналов пикселя, 0..255; кадр совпадает при среднем ≤ 0,005. На одной
  платформе две страницы дают 0 во всех восьми кадрах. `renderGateSeesTheLookGo` в каждом ракурсе: солнце на час позже,
  окна выключены в полночь, тени выключены в полдень.

  | Ракурс | солнце 12→13 | окна | тени |
  |---|---|---|---|
  | city | 0,50 | 0,048 | 0,077 |
  | district | 1,28 | 0,37 | 0,52 |
  | street | 1,76 | 3,17 | 3,29 |
  | corner | 1,90 | 4,05 | 4,68 |

  Наименьшая мутация (окна в `city`) — 9,5 порога. Доля пикселей с разницей больше 16 не годилась: солнце и окна в `city` и
  `district` меняли 0,03–0,35 % пикселей при прежнем пороге 0,5 %.
- Linux x86_64 (образ `mcr.microsoft.com/playwright:v1.63.0-noble`, `--platform linux/amd64` в OrbStack, как CI): кадры
  против финальных эталонов macOS arm64 — среднее 0,0003–0,0015 во всех восьми, меньше трети порога, мутации совпадают с
  macOS до третьего знака; отдельные эталоны по платформам не нужны. Вся спека под эмуляцией: 12 тестов за 3,1 мин.
- Эффекты: всё, кроме GTAO (`?off=ao`): GTAO под SwiftShader рисует чёрное (Q8), его пин — `sceneOcclusionDarkensTheFrameOnTheGpu`.
- Время под SwiftShader: 8 ворот и 4 мутации — 2,3–2,6 мин на 3–4 воркерах при load 11–15; ожидание кадра 240 с, тест 600 с. Первая страница уходит на `about:blank` до второй: обе делят GPU-процесс браузера, и сцена, которая продолжала рисоваться, задерживала сборку шейдеров второй страницы дольше 4 мин.
  `E2E_GPU=1` воротам не нужен. В CI e2e сейчас идёт 5 мин из 30.
...
каскады пересчитываются (`updateFrustums`), глубина коробки тени 40 000, смещение — 0,25 мировой единицы в первом каскаде и
0,5 во втором (three прибавляет его к нормированной глубине; прежние −0,0005 от 40 000 отрывали тень от здания на 20–40
единиц). В ортографических видах каскады доходят до дальней плоскости камеры: пиксель выбирает каскад по глубине до
`maxFar`, а земля под ортокамерой лежит на ~950 единиц вниз по штанге, дальше 900 перспективной стороны, и не затенялась
ни одним каскадом. В карту теней рисуют только корпуса зданий. Каскадов 2, без перекрытия (два пина, `docs/oracle-deviations.md`
2026-09-24).

A/B в одном запуске, тестовый город (`?scenario=city`), тени вкл/выкл через `?off=shadows` по очереди, 3–4 раунда × 5 окон
по 1 с (`.scratch`-спека, в репозиторий не входит):

| Где | Каскады | с тенями | без теней | время кадра |
|---|---|---|---|---|
| веб, GPU, без потолка кадров, 1280×800 | 2, с перекрытием | 340 fps, 2,94 мс | 467 fps, 2,14 мс | +37,4 % |
| то же | 1 | 353 fps, 2,83 мс | 399 fps, 2,51 мс | +13,0 % |
| то же | 2, без перекрытия | 533 fps, 1,88 мс | 584 fps, 1,71 мс | +9,6 % |
| Electron `SIMCITY_TEST_WINDOW=1`, 60 Гц | 2, с перекрытием | 48 fps | 55 fps | +14,6 % |
| то же | 2, без перекрытия | 56 fps | 56 fps | 0 % |

Нагрузка машины 6–35; провалы до 25–47 fps в Electron бывают в обоих режимах.
...
(`fillWindows`), и карта считается нарисованной, когда очередь пуста (`windowsPending` в `renderStats`). Батч выгружает на GPU
только слоты от наименьшего тронутого (`updateRanges`), а не буферы целиком. Процессорное время
