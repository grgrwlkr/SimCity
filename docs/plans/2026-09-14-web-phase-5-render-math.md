...
- Метрика: среднее по кадру наибольшей из разниц каналов пикселя, 0..255; кадр совпадает при среднем ≤ 0,005. На одной
  ...
  |---|---|---|---|
  | city | 0,49 | 0,047 | 0,0129 |
  | district | 1,28 | 0,37 | 0,032 |
  | street | 1,76 | 3,17 | 3,29 |
  | corner | 1,90 | 4,05 | 4,68 |

  Наименьшая мутация (тени в `city`) — 2,6 порога. Доля пикселей с разницей больше 16 не годилась: солнце и окна в `city` и
...
- Linux x86_64 (образ `mcr.microsoft.com/playwright:v1.63.0-noble`, `--platform linux/amd64` в OrbStack, как CI): кадры
  против эталонов macOS arm64 — среднее 0,0003–0,0011 во всех восьми, пятая часть порога; отдельные эталоны по платформам
  не нужны. Проверено на эталонах до отказа от перекрытия каскадов (см. «Тени»). Под эмуляцией прогон шёл 5–6 мин на
  8 тестов, 1–2 теста упирались в ожидание кадров 120 с.
...
единиц). В карту теней рисуют только корпуса зданий. Каскадов 2, без перекрытия (два пина, `docs/oracle-deviations.md`
2026-09-24).
