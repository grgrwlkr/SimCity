...
Прототип-parity сделан (2026-07-12): деревья (spawn-once + Visibility-toggle — despawn-чурн
...
низкое солнце, 5 детерминированных цветов машин. Замеры (один город, ночь): без деревьев 72.4,
деревья+тени 21.9, деревья без теней 58.8 FPS; день при полном солнце ~44 — кандидат №1 для
Tracy (каскады: maximum_distance/резолюция). Осталось из фазы 7: Tracy-перф большого города, синхронизация docs/architecture.md +
