...
> Статус: завершена (фаза 7 закрыта 2026-08-26). Детальные TDD-планы писались по одной фазе за раз
...
### Фаза 7 — полиш и зачистка (✅ 2026-08-26)
...
Прототип-parity сделано (2026-07-12): деревья (spawn-once + Visibility-toggle — despawn-чурн
переиспользовал entity ids и сдвигал сим-тайминги, см. soak; NotShadowCaster — 800 кастеров
роняли кадр 72→22 FPS ночью), объёмные светофоры, чистая разметка (стрелки каждый 4-й тайл),
низкое солнце, 5 детерминированных цветов машин.

Завершение фазы (2026-08-26):
- **Перф-замеры через BRP** (`--release --features dev`, telеметрия `bevy_debugger/debug_dump`,
  фиксированные часы через Space-паузу): test city (~85 зданий, ~210 машин), машина idle:
  пауза-полдень 130 FPS / 9.2 мс, пауза-полночь 121 / 12.9 мс, X3-раннинг (60 тиков/с) ~51 / 34 мс.
  Вывод: рендер после NotShadowCaster-фикса уже под бюджетом 60 FPS; доминирующая стоимость
  на X3 — сим, не каскады. МЕРИТЬ МОЖНО ТОЛЬКО НА IDLE-МАШИНЕ: occlusion окна (не frontmost)
  давит FPS до ~60 и врёт; живой десктоп пользователя добавляет шум.
- **Каскады 4 → 2** (`simcity_frontend/src/game/camera.rs`): орто-камера на буме 500 — вся
  видимая полоса глубин ~[150, 850], два каскада (`num_cascades: 2, first_cascade_far_bound:
  650, maximum_distance: 850`) режут shadow-проходы вдвое и дают БОЛЕЕ резкие ближние тени.
  Зеркалировано в `examples/pseudo3d_proto.rs`.
- **`day_night.ron` — настоящий конфиг**: `night_darkness` (бывш. `max_night_alpha`),
  `sun_day_illuminance`, `ambient_day_brightness`, `window_glow`, `marking_glow`,
  `light_pool_glow`; дефолты = старым hardcoded константам (бит-в-бит), serde-defaults на
  каждом поле.
- **Зачистка 2D-нейминга**: последний `Sprite` в коде — только комментарии; все переименованы
  в quad/mesh/tint-терминологию (glyphs.rs, decay.rs, public_transport.rs, parking.rs,
  traffic.rs, intersections/render.rs, map/render.rs, services/components.rs).
- **Docs sync**: README (рендер-буллет, PgUp/PgDn, F10-ручки), `architecture.md` (секция
  «Рендер»), `ui-architecture.md` (секция камеры переписана под CameraRig), `shortcuts.rs`
  (PgUp/PgDn в панели биндов).
- **Судьба прототипа**: `examples/pseudo3d_proto.rs` остаётся витриной; правки света/камеры
  зеркалируются (CI компилирует его clippy -D warnings).
- Из незакрытого: Tracy-разбор сим-стоимости на X3 (вне рендер-скопа фазы) и палитра-полиш —
  по мере надобности.
