...
- **Поправка 2026-09-11 (после этапа 2b): всё, что можно улучшить при переносе, улучшаем обязательно.** Дефекты Rust не копируются бит в бит, в том числе заторы, платформенно-зависимый float (`powf` libm) и причуды движка (порядок архетипов Bevy). Фикстуры Rust работают как регрессионные ворота, пока его поведение правильное. Улучшение, расходящееся с фикстурой, перегенерирует или снимает её и записывается в план этапа с причиной.
