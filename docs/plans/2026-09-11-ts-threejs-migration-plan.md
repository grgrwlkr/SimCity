...
|---|---|---|---|---|
| 0 | Скелет | `simcity_data/determinism.rs` (3), `no_thread_rng_guard.rs` (1), `sim.rs` (8) | монорепо `packages/{sim,bridge,render,ui,app}`, воркер, fixed-step, RNG, `fingerprint`, очередь команд, `__sim`, CI | фингерпринт двух прогонов в Chromium одинаков на 10 000 тиков пустой карты |
...
| 3½ | Масштаб | — (решение пользователя 2026-09-12) | время 1:1 и лестница скоростей, жители в SoA, машина в кармане и парковки, мезо-трафик и пешеходы, большая карта, устойчивость воркера | миллион жителей держит ×10 на M2; сутки часов пик без распада; фингерпринт двух прогонов в Chromium совпадает |
...
| 5 | Рендер | `render_primitives.rs` (8), `atlas.rs` (9), `buildings/visual.rs` (6), `day_night.rs` (7), `camera_projection.rs` (7), `render_settings.rs` (8), `vignette.rs` (4), `map/render.rs` (5), `map/preview.rs` (9) | инстансированный псевдо-3D, процедурный атлас, день-ночь, пост-обработка, камера с коэффициентами из `render.ron` | скриншоты в Chromium, два прогона, на 4 ракурсах × 2 времени суток, diff с Rust-эталоном ≤ порог, назначенный пользователем по первому сравнению |
...
- **Rust в порте не собирается**, кроме Tauri-крейта `packages/desktop/src-tauri` и того, что нужно для упаковки. Bevy-workspace в корне и корневой `target/` в worktree порта — мусор: не собирать, удалять.
...
| Float-недетерминизм между движками | целочисленная тайловая арифметика; фингерпринт двух прогонов в Chromium в CI с этапа 0. WebKit отдельно не проверяется (решение пользователя 2026-09-14); собранный `.app` на macOS исполняется в WKWebView, так что его проверка и есть проверка в WebKit |
...
| `SharedArrayBuffer` требует COOP/COEP | заголовки в `vite.config.ts` для dev. В Tauri по умолчанию изоляции нет: на `tauri://` WebKit её не даёт даже с COOP/COEP (замер этапа 7). Поэтому собранное приложение отдаёт страницу с `http://127.0.0.1:45174`. Для публичного веба — ограничение на хостинг, записано в README |
| Steam из Tauri | вне плана; Steamworks решается после этапа 7 отдельно |

## Защита кода от разбора

Полной защиты нет: всё, что исполняется у игрока, можно извлечь. Tauri пишет об этом прямо (`security/future.mdx`, раздел «Binary Analysis»): извлечь фронт-ассеты из бинарника «should be a straightforward process». Цель — сделать разбор дорогим, а не невозможным.

| Ступень | Что даёт | Цена |
|---|---|---|
| 1. Vite minify, sourcemap выключен, ассеты сжаты внутри бинарника Tauri (всё — release по умолчанию) | внутренние имена стёрты, JS читается как минифицированный, открытого JS в бинарнике нет | 0, делается на этапе 7 |
| 2. `javascript-obfuscator` только на чанки `packages/ui` и `packages/app`, не на `sim` и `render` | защита от случайного чтения; снимается за часы (оценка) | по README пакета код медленнее на 15–80 % в зависимости от опций, `controlFlowFlattening` — до 1,5×, VM-режим «heavily affects performance»; на горячем пути неприемлемо |
...
Замер ступени 1 на сборке `bun run desktop:build` (2026-09-14). В `packages/app/dist` нет ни `.map`, ни `sourceMappingURL`. `index-*.js` — 595 строк, самая длинная 261 351 символ, имён вроде `installGamepad` и `DebugRenderer` в чанке нет. В бинарнике нет строк `SharedArrayBuffer` и `WebGPURenderer`: фича `compression` входит в `default` крейта `tauri` 2.11.5.

В текущей оболочке у ступени 1 две дыры, их закрывать до релиза:
- Страница отдаётся с `http://127.0.0.1:45174`, иначе WebKit не даёт cross-origin isolation (`2026-09-14-web-phase-7-shell.md`). Пока игра запущена, любой локальный процесс забирает те же чанки обычным `curl`: замер — `/assets/index-*.js` отдаётся с кодом 200, 1 083 617 байт. Сжатие в бинарнике от этого не защищает. Открыть тот же адрес в браузере с DevTools, по-видимому, тоже можно (не проверял).
- `window.__sim`, полный API команд мира, есть и в релизной сборке. Его стоит оставить только для dev и e2e.

Решение по умолчанию: ступень 1 на этапе 7 плюс лицензия. Ступень 2 — отдельный флаг сборки, по требованию. Ступени 3 и 4 — только по явному решению пользователя, они меняют план миграции. Не применимы `bun build --compile --bytecode` и `bytenode`: игра живёт в webview, а не в Bun или Node.
