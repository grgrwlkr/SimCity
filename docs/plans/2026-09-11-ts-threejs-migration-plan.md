...
**Стек:** Vite + TypeScript strict, bun; Three.js (`WebGPURenderer`, откат на WebGL2); React + zustand; zod для конфигов и сейвов; Vitest; Playwright; Electron 44 и electron-builder.

**Спецификация:** Rust-код этого же репозитория в теге `rust-final` (= `edad8fb`; из дерева удалён 2026-09-15, читается через `git show rust-final:<path>`) как источник алгоритмов и тестов, не как эталон (поправка ниже): `crates/`, `src/`, `docs/architecture.md` → «Intersection Traffic Invariants (STRICT)», `docs/gameplay.md`, `docs/persistence.md` — в теге, `assets/config/*.ron` — в дереве, и прежде всего его тесты — они портируются первыми и становятся тестами порта.
...
- Порт живёт в репозитории SimCity: bun-монорепо в корне, каждый этап идёт в ворктри, отведённом от `main`, отдельного репозитория нет. Rust-кода в дереве нет с 2026-09-15: источник алгоритмов и тестов — тег `rust-final`, он только читается, ничего из него в дерево не возвращается.
...
| 7 | Упаковка | `live/*` (89, портируются как Playwright-хелперы) | Electron 44 (Chromium 152), геймпад, файловые сейвы, релизная сборка, minify без sourcemap, рендерер в `app.asar` | сборка `.app`/`.exe`, 60 fps на тестовом городе на M2 |
...
- **Rust в порте не собирается вообще** (решение пользователя 2026-09-14, оболочка — Electron). Bevy-workspace в корне и любой `target/` в worktree порта — мусор: не собирать, удалять.
...
| Float-недетерминизм между движками | целочисленная тайловая арифметика; фингерпринт двух прогонов в Chromium в CI с этапа 0. WebKit отдельно не проверяется (решение пользователя 2026-09-14); десктоп — Electron, тот же Chromium |
...
| `SharedArrayBuffer` требует COOP/COEP | заголовки в `vite.config.ts` для dev. В Electron страница идёт по привилегированной схеме `app://`, и `protocol.handle` ставит COOP/COEP (замер этапа 7: `crossOriginIsolated: true`). Для публичного веба — ограничение на хостинг, записано в README |
| Steam из Electron | вне плана; Steamworks решается после этапа 7 отдельно |

## Защита кода от разбора

Полной защиты нет: всё, что исполняется у игрока, можно извлечь. В Electron рендерер лежит в `app.asar` — архиве без шифрования; ASAR integrity (фьюз `EnableEmbeddedAsarIntegrityValidation`, по умолчанию выключен) проверяет целостность архива, но не прячет код. Цель — сделать разбор дорогим, а не невозможным.

| Ступень | Что даёт | Цена |
|---|---|---|
| 1. Vite minify, sourcemap выключен, рендерер упакован в `app.asar` (всё — release по умолчанию) | внутренние имена стёрты, JS читается как минифицированный | 0, делается на этапе 7 |
