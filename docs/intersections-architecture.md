...
| --- | ----------------------- | -------------- | ------------- | ---------- | ----------- | --------------------- |
| 1.1 | Остановка на красный    | 🔴 High         | Высокая       | Очень выс. | —           | ✅ Выполнено (2025-01) |
| 1.2 | Светофоры в pathfinding | 🔴 High         | Низкая        | Среднее    | —           | ✅ Выполнено (2025-01) |
| 2.1 | Жёлтый сигнал           | 🟡 Medium       | Низкая        | Среднее    | —           | ✅ Выполнено (2025-01) |
| 2.2 | Правила приоритета      | 🟡 Medium       | Средняя       | Высокое    | 1.1         | ✅ Выполнено (2025-01) |
| 2.3 | Стрелки светофора       | 🟡 Medium       | Средняя       | Среднее    | 2.1         | 🔲 Не реализовано      |
| 3.1 | Адаптивные светофоры    | 🟢 Low          | Высокая       | Среднее    | 1.1, 1.2    | 🔲 Не реализовано      |
| 3.2 | Координация светофоров  | 🟢 Low          | Высокая       | Среднее    | 3.1         | 🔲 Не реализовано      |
| 3.3 | Круговое движение       | 🟢 Low          | Высокая       | Высокое    | —           | 🔲 Не реализовано      |
| 3.4 | Пешеходные переходы     | 🟢 Low          | Средняя       | Низкое     | 2.1         | 🔲 Не реализовано      |
| 3.5 | Многоуровневые развязки | 🟢 Low          | Очень высокая | Высокое    | —           | 🔲 Не реализовано      |
| 4.1 | ИИ-оптимизация          | 🔵 Experimental | Очень высокая | Среднее    | 3.1         | 🔲 Не реализовано      |
| 4.2 | V2I коммуникация        | 🔵 Experimental | Высокая       | Низкое     | 1.1         | 🔲 Не реализовано      |
...
✅ **Остановка машин на красный свет** (реализовано)  
✅ **Учёт светофоров в pathfinding** (реализовано)  
✅ **Жёлтый сигнал светофора** (реализовано)  
✅ **Правила приоритета перекрёстков** (реализовано)  
...
1. **Стрелки светофора** — визуальные указатели направления движения
2. **Адаптивные светофоры** — автоматическая оптимизация тайминга
3. **Координация светофоров** — синхронизация между перекрёстками
