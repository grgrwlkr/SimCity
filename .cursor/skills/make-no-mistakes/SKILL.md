...
This skill instructs any model to append the directive **"MAKE NO MISTAKES."** to every user prompt it receives before generating a response.
...
**Response behavior:** Work through the multiplication carefully (17 x 40 = 680, 17 x 3 = 51, total = 731) before answering.

## Notes

- This skill applies to **every prompt** in the session - there are no exceptions.
- The directive should raise the model's internal bar for confidence before outputting anything.
- It does not change the model's tone or style - only its diligence and self-checking behavior.
