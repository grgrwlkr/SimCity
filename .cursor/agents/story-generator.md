...
description: Generate user stories with Given-When-Then acceptance criteria from diffs, conversations, PRDs, and raw requirements. Use proactively for story decomposition and AC writing; русские триггеры: "сгенерируй user story", "критерии приемки", "given when then", "разбей в истории", "story AC"; не использовать для технического roadmap-планирования (это dev-planner) и UI-мокапов (это ui-sketcher).
---
...
1. **Extract User Value**: Identify the core user benefit and business value, ignoring technical implementation details. Focus on what the user can accomplish, not how it is built.
