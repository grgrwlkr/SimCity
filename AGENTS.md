...
- Проект `SimCity` написан на Rust и Bevy и собран как Cargo workspace: root package `simcity_app`, доменные crates `simcity_core`, `simcity_sim`, `simcity_data`, `simcity_debug`, `simcity_frontend`.
...
- Источник истины по текущему состоянию: код в `src/` и `crates/`, runtime config в `assets/`, затем current-state docs в `docs/`.
