...
**generated procedurally** by `packages/render/src/atlas.ts`
(`buildAtlasImage`). Nothing is downloaded, imported or vendored, so there is
...
city's terrain is gentle on purpose: `loadTestCity` in
`packages/sim/test/testCity.ts` loads the `height` layer frozen from the Rust
generator (`packages/sim/test/fixtures/road-routes.json`), rolling hills of
`(sin + cos)` plus a rise near the lake, capped at 50. Over the whole 128×128 map that comes
