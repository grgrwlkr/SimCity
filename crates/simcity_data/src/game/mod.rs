...
mod bus_seeding_tests {
    /// Mass-freeze pin: by day 18 the city must NOT accumulate permanently-stuck traffic. Guards
    /// the day-11+ gridlock class where one vehicle wedged at an intersection (e.g. a bus whose
    ...
    fn no_mass_freeze_by_day_18() {
        ...
        let mut last_move: HashMap<u64, (crate::game::map::TilePos, usize)> = HashMap::new();
        let mut worst = 0usize;
        for t in 0..4400usize {
            tick(&mut app, 1);
            if t % 100 != 0 {
                continue;
            }
            ...
            let rows: Vec<(
                u64,
                simcity_sim::game::transport::PathHandle,
                usize,
            ...
            for (id, h, cursor, spawn_tile) in rows {
                ...
                if len <= 1 {
                    last_move.remove(&id);
                    continue; // idle/arrived — not traffic
                }
                let cur = pool
                    .get_tile(h, cursor.min(len.saturating_sub(1)))
                    .unwrap_or(spawn_tile);
                let entry = last_move.entry(id).or_insert((cur, t));
                if entry.0 != cur {
                    *entry = (cur, t);
        ...
        assert!(
            worst <= 6,
            "mass freeze: {worst} vehicles stuck >=60s at peak (day-18 horizon) — a wedged \
